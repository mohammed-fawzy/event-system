<template>
  <div class="card flex justify-center">
      <Dialog :visible="visible" modal :header="id ? $t('common.editForm', { name: $t('table.organizationLegal')}) : $t('common.addNewForm.female', { name: $t('table.organizationLegal') })"  @update:visible="closeModal" :style="{ width: '50rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
          <Form @submit="onSubmit" v-slot:default="{ errors }">
            ...
            <div class="flex justify-between mb-8 gap-9">
              ...
              <CustomeTextInput v-model="theData.code" name="name" :label="$t('common.code')" :errors="errors" :rules="'required'"/>
            </div>
            <div class="flex flex-col basis-2/4">
              <CustomeTextInput v-model="theData.abbreviationLetter" name="name" :label="$t('table.abbreviationLetter')" :errors="errors" :rules="'required'"/>
            </div>
            </div>
            <div class="flex flex-col mb-8"> 
